...
int main() {
    ...
    if (input.is_open()) {
        ...
        sort(signals.begin(), signals.end(), Comparator());
        for (string& l : signals)
            cout << l << endl;


        long index2 = find(signals.begin(), signals.end(), "[[2]]") - signals.begin() + 1;
        long index6 = find(signals.begin(), signals.end(), "[[6]]") - signals.begin() + 1;

        cout << "Decoder key is " << index2*index6 << endl;
    } else {
